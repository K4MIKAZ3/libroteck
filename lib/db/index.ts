...
import { productPrices, products, settings, stores } from "./schema";
...
async function seedIfEmpty(db: ReturnType<typeof drizzle<typeof schema>>) {
  ...
  const existingProducts = await db
    .select()
    .from(products)
    .where(eq(products.storeId, libroteckStore.id))
    .limit(1);
  ...
  const existingSettings = await db
    .select()
    .from(settings)
    .where(eq(settings.storeId, libroteckStore.id))
    .limit(1);

  if (existingSettings.length === 0) {
    await db.insert(settings).values({
      storeId: libroteckStore.id,
